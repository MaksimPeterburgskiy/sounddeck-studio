...
8. Test actual driver installation only in a disposable Windows VM. Confirm the missing-driver, already-installed, helper-failure, signature-failure, custom-install-path, update, and reboot flows. Driver setup runs whenever the driver is missing, including during application updates, and executes from NSIS-private storage; the security boundary must remain independent of `$INSTDIR`.
...
This chain protects the build inputs, not the delivered installer. Published SoundDeck Studio installers are currently unsigned, so a setup executable tampered with after release bypasses every check described here. The installer-time protections only defend the window between extraction and driver execution on the target machine. Signing the installer is the remaining gap.
